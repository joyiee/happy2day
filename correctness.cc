...
private:
    const uint64_t SIMPLE_TEST_MAX = 256;
    ...
    void regular_test(uint64_t max)
    {
        ...
        for (i = 0; i < max; i++) {
            store.put(i, std::string(i+1, 's')+to_string(i));
            EXPECT( std::string(i+1, 's')+to_string(i), store.get(i));
        }
        ...
        {
            EXPECT(std::string(i+1, 's')+to_string(i), store.get(i));
            //EXPECT(string(i,'l'),store.get(514-i));
        }
...
*/
        ...
        for (i = 0; i < max; i+=2)
            EXPECT((i & 1) ?  std::string(i+1, 's')+to_string(i) : not_found,
                   store.get(i));
